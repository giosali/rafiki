...
void Crud::CreateIcon(const std::shared_ptr<Result>& result) {
...
void Crud::CreateWebSearch(const std::shared_ptr<WebSearch>& web_search) {
  auto user_settings = Io::GetFile(Paths::Ini::kUser);
  ...
  CreateResult(web_search);
  CreateIcon(web_search);

  // Adds web search to the document's array.
  auto document = Io::GetFile(Paths::Json::kYourWebSearches);
  auto array = document.array();
  array.append(web_search->ToJsonObject());
  document.setArray(array);
  UpdateYourWebSearches(document);
}

void Crud::DeleteWebSearch(const Id& id) {
  ...
  DeleteResult(web_search);
  UpdateYourWebSearches();
}

void Crud::Initialize() {
  // Sets up default settings.
  auto default_settings = Io::GetFile(Paths::Ini::kDefault);
  auto user_settings = Io::GetFile(Paths::Ini::kUser);
  ...
  CreateResult<WebSearch>(Paths::Json::kWebSearches);
  CreateResult<WebSearch>(Paths::Json::kYourWebSearches);
...
std::vector<std::shared_ptr<Result>> Crud::ReadDefaultResults() {
  return default_results_;
}

std::shared_ptr<Result> Crud::ReadResult(const Id& id) {
  auto it = results_.find(id);
  return it == results_.end() ? nullptr : it->second;
}

std::vector<std::shared_ptr<Result>> Crud::ReadResults(const Input& input) {
...
void Crud::ToggleDesktopEntry(bool create) {
...
void Crud::ToggleResult(const Id& id, bool enable) {
  auto it = results_.find(id);
  if (it == results_.end()) {
    return;
  }
...
void Crud::UpdateWebSearch(const std::shared_ptr<WebSearch>& web_search,
...
template <typename T>
void Crud::CreateResult(Paths::Json f) {
  auto document = Io::GetFile(f);
  if (document.isObject()) {
    CreateResult(std::make_shared<T>(document.object()));
  } else if (document.isArray()) {
    for (const auto& i : document.array()) {
      CreateResult(std::make_shared<T>(i.toObject()));
    }
  }
}

void Crud::CreateResultHelper(const std::shared_ptr<Result>& result) {
...
void Crud::DeleteResult(const std::shared_ptr<Result>& result) {
  auto id = result->GetId();
...
void Crud::UpdateYourWebSearches(const QJsonDocument& document) {
...
void Crud::UpdateDefaultResults() {
...
Autocompleter Crud::autocompleter_;

std::vector<std::shared_ptr<Result>> Crud::default_results_;

std::unordered_set<Id> Crud::disabled_ids_;

std::vector<std::shared_ptr<ProcessedResultBuilder>>
  Crud::processed_result_builders_;

std::vector<std::shared_ptr<ProcessedResult>> Crud::processed_results_;

std::unordered_map<Id, std::shared_ptr<Result>> Crud::results_;
